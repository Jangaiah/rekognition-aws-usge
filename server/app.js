...
import { fixContrast } from "./aws-rekognition/aws-rekongnition-color-contrast.js";
...
app.post("/api/enhance-contrast", upload.single("file"), async (req, res) => {
  try {
    if (!req.file || !req.file.buffer) {
      return res.status(400).json({ error: "No file uploaded or file is invalid." });
    }

    const resImage = await fixContrast(req.file.buffer);
    if(resImage?.fixedImageBuffer) {
      
      res.set('Content-Type', 'image/png');
      res.send(resImage);
    } else {
      res.json({message: resImage?.message});
    }
  } catch (err) {
    console.error("Error processing image:", err);
